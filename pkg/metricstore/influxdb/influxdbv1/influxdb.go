...
func (s *Storage) WriteOnDemandMetric(dbName, metricName string, tagArr map[string]string, metricVal map[string]interface{}) error {
...
func (s *Storage) ReadMetric(isPUSH bool, vmId, metric, period, aggregateType, duration string) (interface{}, error) {
	...
	queryString, err := s.buildQuery(isPUSH, vmId, metric, period, aggregateType, duration)
	if err != nil {
		return nil, err
	}
...
func (s Storage) buildQuery(isPUSH bool, vmId string, metric string, period string, aggregateType string, duration string) (string, error) {

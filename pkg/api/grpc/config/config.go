...
type GrpcServiceList struct {
	DragonflySrv *GrpcServerConfig `mapstructure:"dragonflysrv"`
...
func (gConf *GrpcConfig) initGlobalParams() {
	...
	if gConf.GSL.DragonflySrv != nil {

		if gConf.GSL.DragonflySrv.TLS != nil {
			if gConf.GSL.DragonflySrv.TLS.TLSCert != "" {
				gConf.GSL.DragonflySrv.TLS.TLSCert = ReplaceEnvPath(gConf.GSL.DragonflySrv.TLS.TLSCert)
			}
			if gConf.GSL.DragonflySrv.TLS.TLSKey != "" {
				gConf.GSL.DragonflySrv.TLS.TLSKey = ReplaceEnvPath(gConf.GSL.DragonflySrv.TLS.TLSKey)
			}
		}

		if gConf.GSL.DragonflySrv.Interceptors != nil {
			if gConf.GSL.DragonflySrv.Interceptors.Opentracing != nil {
				if gConf.GSL.DragonflySrv.Interceptors.Opentracing.Jaeger != nil {

					if gConf.GSL.DragonflySrv.Interceptors.Opentracing.Jaeger.ServiceName == "" {
						gConf.GSL.DragonflySrv.Interceptors.Opentracing.Jaeger.ServiceName = "grpc dragonfly server"
					}

					if gConf.GSL.DragonflySrv.Interceptors.Opentracing.Jaeger.SampleRate == 0 {
						gConf.GSL.DragonflySrv.Interceptors.Opentracing.Jaeger.SampleRate = 1
					}
			...
			if gConf.GSL.DragonflyCli.Interceptors.Opentracing != nil {
				if gConf.GSL.DragonflyCli.Interceptors.Opentracing.Jaeger != nil {
					...
					if gConf.GSL.DragonflyCli.Interceptors.Opentracing.Jaeger.ServiceName == "" {
						gConf.GSL.DragonflyCli.Interceptors.Opentracing.Jaeger.ServiceName = "grpc dragonfly client"
					}
